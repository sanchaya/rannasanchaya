...
desc "import kavirajamarga from csv"
task :import_kaviraja_from_csv => :environment do
  file_name = Rails.root.to_s + "/lib/kavi.csv"
  puts "started"
  @author = Author.create(name: 'ಶ್ರೀವಿಜಯ')
  CSV.foreach(file_name, :col_sep => ",", :headers => false) do |row|
    Poem.create(poem_text: row[1], author_id: @author.id)
    p ">>>>>>>>>>>>>>>>>>>>>"
  end
